...
def sentence_to_tensor(word2index, list, max_length):
    ...
    # print(len(input_length, len(input_list)))
...
def image_feature_tensor(feature_data, label, max_length):
    ...
    for i in range(len(feature_data)):
        ...
        input_total.append(feature_data[i][0] + feature_data[i][1] + feature_data[i][2] +
                           [[0, 0, 0, 0, 0, 0, 0, 0, 0]] * (max_length * 3 - len(feature_data[i][0])
                                                                           - len(feature_data[i][1])
                                                                           - len(feature_data[i][2])))

    # print(len(input_0), len(input_0_len) )
    # print(len(input_1), len(input_1_len) )
    # print(len(input_2), len(input_2_len))
    input_0 = torch.tensor(input_0, dtype=torch.float, device=device).view(len(input_0), -1, 9)
    input_1 = torch.tensor(input_1, dtype=torch.float, device=device).view(len(input_1), -1, 9)
    input_2= torch.tensor(input_2, dtype=torch.float, device=device).view(len(input_2), -1, 9)
    input_total = torch.tensor(input_total, dtype=torch.float, device=device).view(len(input_total), -1, 9)
    ...
    input_total_len = torch.tensor(input_total_len, dtype=torch.long, device=device).view(-1, 1)
    label = torch.tensor(label, dtype=torch.long, device=device).view(-1, 1)

    return input_0, input_1, input_2, input_total, input_0_len, input_1_len, input_2_len, input_total_len, label
